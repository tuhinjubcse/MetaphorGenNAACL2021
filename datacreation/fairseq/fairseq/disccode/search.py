...
class BeamSearch(Search):
    ...
    def step(self, step, lprobs, scores, src_tokens=None, gen_tokens=None, **kwargs):
        ...
        if rescore and step > 0 and (gen_tokens[0][-1] in [4,479]):
            #print("I am here.....",gen_tokens.shape)
            garbage , top_indices = torch.topk(lprobs, beam_size)
            top_indices = top_indices.squeeze(0)
            n_hypos = top_indices.shape[1]
            #print("Shape is",n_hypos)
            ...
            for coef, scorer in zip(coefs, scorers):
                all_tokens = gen_tokens if step > 0 else src_tokens
                # all_tokens = all_tokens.repeat_interleave(n_hypos, dim=0)
                hypothesis_batch = torch.cat((all_tokens, top_indices.transpose(0, 1)), dim=1)
                if hypothesis_batch.shape[1] > 512:
                    hypothesis_batch = hypothesis_batch[:,:-512]
                gold_separate = False
                if learn and learn_every_token:
                    gold_example = gold_tokens
                    if gold_example.shape[1] == hypothesis_batch.shape[1]:
                        hypothesis_batch = torch.cat((hypothesis_batch, gold_example))
                    else:
                        gold_separate = True
                new_scores = scorer.predict("sentence_classification_head", hypothesis_batch)
                ...
                raw_scores = np.array([score[1].data.item() for score in new_scores])
                all_raw_scores.append(raw_scores)
                score_adjustment += raw_scores[:beam_size] * coef
            ...
            max_lprob, max_idx = lprobs.max(2)
            if learn and learn_every_token:
                next_gen_lm_score = torch.sum(torch.cat((max_lprob[0], gen_lm_score.unsqueeze(0))))
                gold_cont_raw_scores = all_raw_scores[-1]
                loss = coef_trainer.train_coefficients(gold_lm_score, next_gen_lm_score,gold_cont_raw_scores,all_raw_scores[max_idx.data.item()])
...
class Sampling(Search):
    ...
    def step(self, step, lprobs, scores, src_tokens=None, gen_tokens=None, **kwargs):
        ...
        #print("step is",step)
        #print("Gen tokens are",gen_tokens)
        ...
        if rescore and step > 0:
            ...
            for coef, scorer in zip(coefs, scorers):
                ...
                raw_scores = np.array([score[1].data.item() for score in new_scores])
                all_raw_scores.append(raw_scores)
                #print("all raw scores",all_raw_scores)
                score_adjustment += raw_scores[:self.sampling_topk] * coef
            all_raw_scores = np.stack(all_raw_scores, axis=-1)
            mod_probs = lprobs.clone()
            for i in range(n_hypos):
                mod_probs[0][0][i] = lprobs[0][0][i] + score_adjustment[i]
