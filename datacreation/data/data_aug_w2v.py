...
mlm = RobertaForMaskedLM.from_pretrained("roberta-large")
...
def loadData(f):
    ...
    for line in tqdm(f_csv):
        ...
        data.append((ids, new_verb_posi, sentence, verb_posi, verb))
    return data, p, n
...
def augmentation(dataset):
    data = []
    for ids, new_verb_posi, sentence, verb_posi, origin_verb in tqdm(dataset):
        ...
        for each in pred:
            if each != word_id:
                ...
                # ensure the prediction is verb
                pos = nltk.pos_tag(sentence)
                if pos[verb_posi][1] not in ["VB", "VBG", "VBD", "VBN", "VBP", "VBZ"]:
                    continue

                verb_lemmatized = lemmatizer.lemmatize(verb, "v")
                if origin_verb in w2v:
                    if verb_lemmatized not in w2v:
                        continue
                    if w2v.similarity(verb_lemmatized, origin_verb) > threshold:
                        continue
                print(origin_verb, verb_lemmatized, sep=" --> ")
                data.append((None, None, verb_lemmatized, " ".join(sentence), verb_posi, 0))
                break
    return data
...
with open("VUA_verb/data_augmentation_train_w2v.csv", "w", encoding="utf-8") as f:
    storeData(f, augmentation(train_dataset))
with open("VUA_verb/data_augmentation_valid_w2v.csv", "w", encoding="utf-8") as f:
