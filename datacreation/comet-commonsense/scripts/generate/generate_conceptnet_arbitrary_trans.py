...
for sentence,verb,position,scores in tqdm(data):
    words = sentence.split()
    replaceword = words[position]
    literal = sentence
    e1 = literal
    r =  "SymbolOf"
    ...
    for m in scores[0:25]:
        ...
        overlap = len(set(literal_symbol).intersection(set(metaphor_symbol)))
        if overlap > 3:
            intersec_scores.append((meta,overlap,m[1]))
    if intersec_scores:
        intersec_scores.sort(key = lambda x: (x[1],x[2]),reverse=True)
        m = {"literal": literal, "position": position, "verb": replaceword ,"symbol_lit": literal_symbol,"metaphors": intersec_scores,}
