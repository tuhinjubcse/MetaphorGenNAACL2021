...
class BARTHubInterface(nn.Module):
    ...
    def decode(self, tokens: torch.LongTensor):
        ...
        #for s in sentences:
            #print(self.model,self.task,self.args)
            #print("dictionary is",self.task.source_dictionary.string(s))
    ...
    def generate(self, tokens: List[torch.LongTensor], beam: int = 5, verbose: bool = False, **kwargs) -> torch.LongTensor:
        ...
        translations = self.task.inference_step(
            ...
            [self.model],
            sample,
            prefix_tokens=sample['net_input']['src_tokens'].new_zeros((len(tokens), 1)).fill_(self.task.source_dictionary.bos()),**kwargs
        )
